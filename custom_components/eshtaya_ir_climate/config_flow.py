    ...
    async def inspect(device: dict[str, Any]) -> dict[str, Any] | None:
        ...
        async with semaphore:
            try:
                ...
                # If device-level functions are hidden/empty, try the
                # category instruction set before the verified DP fallback.
...
async def _validate_manual_device(
    ...
    if not caps.climate_compatible:
        _LOGGER.warning(
            "Manual Tuya IR climate validation rejected device %s: "
            "category=%s, live_status_codes=%s, specification_dp_codes=%s",
            f"...{device_id[-6:]}" if len(device_id) > 6 else device_id,
            device.get("category") or spec.get("category"),
            sorted(live_status),
            sorted(caps.dps),
        )
