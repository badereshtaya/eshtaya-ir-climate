...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    for device_id in entry.data[CONF_DEVICE_IDS]:
        ...
        category = str(
            probe.category
            or probe.device.get("category")
            or ""
        )
        ir_library = IrCommandLibrary(
            hass,
            device_id,
            known_profile=(category == "hwktwkq"),
        )
        await ir_library.async_load()
        ...
        # Import recent Smart Life commands first, then start normal updates.
        # Failure of log APIs never blocks the integration.
        try:
            await coordinator.async_bootstrap_ir_learning()
        except TuyaApiError:
            pass
