...
class TuyaOpenApi:
    ...
    async def get_specification(self, device_id: str) -> dict[str, Any]:
        """Get the effective device instruction/status specification.

        Tuya's v1.2 IoT-03 endpoint describes the standard instruction/status
        set. A product configured for "DP Instruction" can validly return an
        empty standard set there. In that case we must continue to the legacy
        per-device specification/functions APIs, which reflect the effective
        device instructions that can actually be sent.
        """
        ...
        for path in specification_paths:
            ...
            if (
                isinstance(functions, list) and len(functions) > 0
            ) or (
                isinstance(status, list) and len(status) > 0
            ):
                return result

        # Important for products using Tuya "DP Instruction" mode:
        # /v1.0/iot-03/devices/{id}/functions can expose the effective instruction set
        # even when the standard v1.2 specification is empty.
        try:
            function_result = await self._request(
                "GET",
            ...
            if isinstance(function_result, dict):
                functions = function_result.get("functions")
                if isinstance(functions, list) and functions:
                    merged = dict(best or {})
                    merged["category"] = (
                        function_result.get("category")
                        or merged.get("category")
                    )
                    merged["functions"] = functions

                    # If the status metadata is unavailable, build a readable
                    # status shell from the live status codes. DP metadata from
                    # writable functions is retained when parsing duplicates.
                    if not isinstance(merged.get("status"), list) or not merged["status"]:
                        try:
                            live = await self.get_status(device_id)
                        except TuyaApiError:
                            live = {}
                        merged["status"] = [
                            {
    ...
    async def get_category_functions(self, category: str) -> dict[str, Any]:
        """Get the instruction set for a Tuya product category."""
        result = await self._request(
            "GET",
            f"/v1.0/iot-03/categories/{category}/functions",
        )
        if not isinstance(result, dict):
            raise TuyaDeviceError("Invalid category functions response")
        return result
