...
class TuyaOpenApi:
    ...
    async def _request(
        ...
        *,
        ...
        # Tuya signs the canonical URL, and query/form parameters MUST be
        # sorted by key in ascending alphabetical order before signing.
        # Keep comma-separated DP/event lists readable exactly as Tuya's
        # documented request examples do.
    ...
    async def get_operation_logs(
        ...
        *,
        ...
        """Read IoT Core device operation logs.

        Tuya's free-edition operation-log query uses query_type=1.
        Event type 5 is command sending and type 7 is DP reporting.
        """
        query: dict[str, Any] = {
            "type": event_types,
        ...
        if codes:
            query["codes"] = ",".join(str(code) for code in codes if code)

        result = await self._request(
            "GET",
            f"/v2.0/cloud/thing/{device_id}/logs",
            query=query,
        )
        if not isinstance(result, dict):
            return []
    ...
    async def get_report_logs(
        ...
        *,
        ...
        """Read DP report logs, preferring the current IoT Core endpoint."""
        code_string = ",".join(str(code) for code in codes if code)
        if not code_string:
            return []
        ...
        try:
            ...
            # Legacy IoT-03 fallback for older projects/regions.
            result = await self._request(
                "GET",
                f"/v1.0/iot-03/devices/{device_id}/report-logs",
                query=query,
            )
