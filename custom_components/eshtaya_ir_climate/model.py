...
# Verified Tuya DP-Instruction profile for IR Air Thermostat controllers.
# This is only activated when the live device status exposes the distinctive
# combination infrared_switch + target_temp + mode + fan_level.
VERIFIED_IR_THERMOSTAT_PROFILE: dict[str, tuple[str, dict[str, Any], bool]] = {
    "infrared_switch": ("Boolean", {}, True),
    "target_temp": (
        "Integer",
        {"min": 16, "max": 30, "scale": 0, "step": 1, "unit": "℃"},
        True,
    ),
    "mode": (
        "Enum",
    ...
    "temp_current": (
        "Integer",
        {"min": -200, "max": 1000, "scale": 1, "step": 1, "unit": "℃"},
        False,
    ),
    "humidity_current": (
        "Integer",
        {"min": 0, "max": 100, "scale": 0, "step": 1, "unit": "%"},
        False,
    ),
    "child_lock": ("Boolean", {}, True),
    "filter_reset": ("Boolean", {}, True),
    "runtime_total_reset": ("Boolean", {}, True),
    "filter_life": ("Integer", {"scale": 0, "step": 1}, False),
    "runtime": ("Integer", {"scale": 0, "step": 1}, False),
    "status": ("Enum", {"range": ["done", "run", "idle"]}, False),
    "fault": ("Fault", {}, False),
}

VERIFIED_IR_THERMOSTAT_SIGNATURE = {
    "infrared_switch",
    "target_temp",
    "mode",
    "fan_level",
...
def merge_verified_ir_thermostat_profile(
    spec: dict[str, Any],
    live_status: dict[str, Any],
) -> dict[str, Any]:
    """Merge a verified DP-Instruction profile when Tuya hides function metadata.

    Some Tuya products configured for DP Instruction mode expose the DP codes
    in live status while the standard/specification function set remains
    empty. We only synthesize writable metadata when the distinctive verified
    four-DP signature is present, avoiding broad assumptions for unrelated
    devices.
    """
    live_codes = set(live_status)
    if not VERIFIED_IR_THERMOSTAT_SIGNATURE.issubset(live_codes):
        return spec

    merged = dict(spec)
    functions = [
        row for row in _extract_rows(merged, "functions")
        if isinstance(row, dict)
    ]
    status_rows = [
        row for row in _extract_rows(merged, "status")
        if isinstance(row, dict)
    ]
    function_codes = {str(row.get("code", "")) for row in functions}
    status_codes = {str(row.get("code", "")) for row in status_rows}

    for code, (dp_type, values, writable) in VERIFIED_IR_THERMOSTAT_PROFILE.items():
        ...
        if writable and code not in function_codes:
            functions.append(row)
            function_codes.add(code)
        if code not in status_codes:
            status_rows.append(row)
            status_codes.add(code)

    merged["functions"] = functions
    merged["status"] = status_rows
    merged["_eshtaya_profile"] = "verified_ir_air_thermostat_dp_instruction"
    return merged
    ...
    def from_specification(cls, spec: dict[str, Any]) -> "DeviceCapabilities":
        ...
        for row in _extract_rows(spec, "status"):
            ...
            if code in dps:
                ...
                # Keep richer writable-function metadata when the fallback
                # status shell has no DP schema information.
                if values:
                    dps[code].values = values
                if not dps[code].type and row.get("type"):
                    dps[code].type = str(row.get("type"))
            else:
                dps[code] = DpMeta(
                    code=code,
                    type=str(row.get("type", "")),
